...
- `app/src-tauri/src/commands.rs` — the core commands `request`, `attach_terminal`,
  `detach_terminal`, `terminal_input`, `connection`, `layout`, `save_layout`, and `set_visible`.
- `app/src-tauri/src/gui_state.rs` — the GUI state file: `Saved { layout }` per socket path, holding
  dockview's serialized layout as is, and `Connection`.
- `app/src/ipc/` — `request()`, `attachTerminal()`, `detachTerminal()`, `terminalInput()`,
  `connection()`, `layout()`, `saveLayout()`, `setVisible()`, and the `onWatch()`, `onSession()`,
  and `onConnection()` listeners; `bindings/` is generated by `cargo test` and committed.
