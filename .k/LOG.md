...
- Removed redundant per-turn allocations in the agent loop: `EventStream::commit` now moves `pending_history` into the session instead of cloning it (commit is terminal, so the working copy is never read again), and `StreamTool` carries only the tool name rather than a full `ToolSchema` clone, since dispatch reads only the name and the provider-facing schema vector already holds the copy sent to the provider.
- Stopped copying every streamed SSE chunk: `State::Reading` now holds reqwest's `Bytes` directly instead of mapping each chunk into a fresh `Vec<u8>`, and `decode_chunk` destructures the wire tool-call function once so the name moves out instead of being cloned. Added `bytes` as a workspace dependency to name the stream item type.
- Simplified DeepSeek error and finish-reason handling: `error_message` reuses its in-scope canonical-reason fallback instead of recomputing it through a single-use `DefaultIfEmpty` extension trait (removed, along with the `status` parameter that only threaded it through), and the per-chunk finish reason now travels in a dedicated `SseItem::FinishReason` variant rather than a `RawEvent::Done` carrier with a discarded usage field.
- Hoisted a parameter-name allocation out of a `find` closure in the `#[ur::tool]` code generator; the generated tokens are unchanged.
- Considered but did not pursue: unifying the three name/length validators (the macro crate cannot depend on `ur-core`, and the DeepSeek user-id check is a distinct entity with a different length cap); merging the two `max_tokens` validations (provider-agnostic pre-flight vs provider-specific catalog cap are intended defense in depth); and removing the `EventStream::new` / `start_provider_turn` history clones (required by the rollback model and the owned-`Request` boundary). Two marginal idiom cleanups (merging the `encode_message` System/User arms, deduping the macro `finish` fragment) were left alone as not clearly improving clarity and, for the latter, risking churn against the token-snapshot tests.

## Simplification review (second pass)

- A second whole-codebase, behavior-preserving review produced five applied batches, each verified with `cargo fmt`, `cargo clippy --workspace --all-features --all-targets -- -D warnings`, and `cargo test --workspace --all-features`. The live DeepSeek and OpenAI smoke tests were re-run because the streaming byte path changed for both providers.
- `ur-openai`'s executor now streams reqwest `Bytes` directly instead of copying each chunk into an owned `Vec<u8>`, matching `ur-deepseek` and removing one allocation per streamed chunk. Added `bytes` as a dependency of `ur-openai`.
- Trimmed per-event SSE allocations: `dispatch_event` moves the sole data line out when an event carries a single `data:` line rather than always re-joining, and the OpenAI tool-call decode destructures `WireFunction` once instead of reading it twice and cloning the name.
- SSE line decoding classifies each line off a borrowed slice of the buffer into an owned action, then drains the consumed prefix, eliminating the per-line `Vec<u8>` that previously existed only to satisfy the borrow checker. The decoder structure stays identical across both provider crates.
