...
- **`response_format`.** `JsonObject` sends `{ "type": "json_object" }`; the prompt must still ask for JSON. DeepSeek has no schema-constrained response format, so `JsonSchema(..)` surfaces as `Error::Config` on the `send` stream before any network round-trip — use `JsonObject`, or OpenAI for strict structured outputs.
