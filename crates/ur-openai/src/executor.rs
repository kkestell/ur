//! HTTP execution wired to the shared executor: OpenAI's retry policy and
//! error mapping.
...
impl Dialect for Config {
    ...
    async fn status_error(
        ...
        match status {
            ...
            429 if body.code.as_deref() == Some("insufficient_quota") => Error::InsufficientFunds,
...
mod tests {
    ...
    async fn client(server: &MockServer) -> crate::OpenAiClient {
        crate::OpenAiClient::builder()
            .api_key("test-key")
            .base_url(server.uri())
            .max_retries(0)
            .build()
    ...
    async fn collect(client: &crate::OpenAiClient) -> Vec<Result<RawEvent>> {
    ...
    async fn retries_retryable_statuses_then_succeeds() {
        for status in [408, 409, 429, 500, 502, 503, 504] {
            ...
            let client = crate::OpenAiClient::builder()
                .api_key("test-key")
                .base_url(server.uri())
                .max_retries(1)
                .build()
    ...
    #[tokio::test]
    async fn openai_insufficient_quota_429_maps_to_insufficient_funds() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/chat/completions"))
    ...
    async fn malformed_stream_yields_decode_error() {
        ...
        match collect(&client).await.pop().unwrap() {
            Err(Error::Decode { context, .. }) => {
                assert_eq!(context, "decoding OpenAI SSE chunk");
            }
    ...
    async fn eof_before_done_yields_decode_error() {
        ...
        match collect(&client).await.pop().unwrap() {
            Err(Error::Decode { context, source }) => {
                assert_eq!(context, "reading OpenAI SSE stream");
